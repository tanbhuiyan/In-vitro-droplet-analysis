...
# This is written so that all replicates for a given experiment are in a folder together
#
# <> denotes folder
#
# e.g. <PARENT_FOLDER>
...
# this loops over CONDITION FOLDERS
...
for folder in dir_list:
    if not folder.startswith('.') and os.path.isdir(os.path.join(input_params.parent_path, folder)):
        ...
        for idx, file in enumerate(base_name_files):
            ...
            ''' The commented code below is when the metadata file and image files are separate '''
...
#             data = methods.load_images_nd(replicate_files, data, input_params, folder)
            
            data = methods.load_images_czi(os.path.join(input_params.parent_path, folder, file), data, input_params, folder)
